...
class StartCommand(CommandInterface):
    def __init__(self, filter_factory: MessageFilterFactoryInterface, first_state: str, first_keyboard: List[List[str]] = None):
    ...
    async def handle_command(self, message: dict, context: dict, chat: ChatInterface):
        ...
        if self.first_keyboard is None:
            ...
            await chat.send_keyboad(message, self.first_keyboard)
        return chat.move_next(context, self.first_state)
    
    async def wrong_message(self, message: dict, context: dict, chat: ChatInterface):
        keyboard = [["/start"]]
        await chat.send_keyboad("👻 Для начала работы с ботом выполните команду /start (или нажмите соответствующую кнопку)", keyboard)
